"""Single-entry Codex CLI runner across all registered benchmarks.

Replaces the per-bench `run_codex_cli_<bench>.py` files. Picks a
benchmark via `--bench` from the registry in `benchmarks/specs/` and
delegates per-item work to `common.codex_runner`.
"""
...
from omnicoding.benchmarks.common.codex_runner import run_codex_item  # noqa: E402
...
LOGGER = logging.getLogger("benchmarks.harnesses.run_codex")
LOG_FORMAT = "%(asctime)s - %(levelname)s - %(message)s"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Codex CLI across registered benchmarks.")
    add_io_args(parser, default_workspace_subdir=".codex_workspaces")
...
def _resolve_codex_bin(raw: str) -> str:
    """Resolve the codex CLI binary. Defers to PATH for `codex`."""

    candidate = shutil.which(raw) if raw == "codex" else raw
    if candidate is None:
        raise FileNotFoundError(
            "Codex CLI binary not found on PATH. "
            "Pass --codex_bin <absolute path> or install codex.",
        )
    if not Path(candidate).exists():
        raise FileNotFoundError(f"--codex_bin not found: {candidate}")
...
async def _run_batch_loop(
    ...
    """For VideoZeroBench-style benches with `groups_by`. Each group
    becomes a single codex run; we synthesize one virtual "item" that
    carries the whole group via the `__group__` field, then split the
    result into per-question rows after the run."""

    semaphore = asyncio.Semaphore(args.concurrent_limit)
    assert spec.groups_by is not None, "_run_batch_loop requires spec.groups_by"
    ...
    async def _run_group(index: int, members: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        single_row = await run_codex_item(
            item_index=index,
            item=head,
            spec=spec,
            args=args,
        ...
        # Fan out to one row per question_id by repeating the spec's
        # result_row builder for each member while preserving the model
        # output the batch run produced.
...
async def main_async(args: argparse.Namespace) -> None:
    ...
    output_root, results_path, metrics_path = build_run_paths(
        args, dirname=f"{spec.name}_codex_{run_dir}", timestamp=timestamp,
    )
...
def main() -> None:
    ...
    finalize_args(
        ...
        workspace_subdir=".codex_workspaces",
    )
